...
def build_html_report(
    ...
    if has_infer:
        toc_links.append('<a href="#infer-heatmap">Infer Affinity</a>')
        toc_links.append('<a href="#delta">Delta</a>')
        toc_links.append('<a href="#correlation">Correlation</a>')
...
<title>MoE Affinity Analysis — Train / Inference Discrepancy</title>
<style>
...
<div class="chart-container">
  <img src="data:image/png;base64,{{summary_b64}}" alt="Summary Statistics">
</div>

<!-- Train Affinity Heatmap -->
...
<div class="insight-box">
  <strong>How to read:</strong> Cell (i, j) shows how frequently rank i communicates with rank j,
  derived from expert co-activation in the routing trace. Darker cells = more co-activation.{{preset_note}}
</div>
""")
    ...
    html = html.replace("{{summary_b64}}", charts.get("summary", ""))
    ...
    # Inference heatmap
    if has_infer:
        html += f"""<!-- Inference Affinity Heatmap -->
...
<h2 id="js-divergence">Per-Layer Routing Distribution Shift</h2>
<div class="chart-container">
  <img src="data:image/png;base64,{charts.get('js_divergence', '')}" alt="Per-Layer JS Divergence">
</div>
<div class="insight-box">
  <strong>How to read:</strong> Each bar is one MoE layer. Low JS divergence (&lt; 0.1, green) means
  training and inference routing distributions are nearly identical &mdash; preset circuits
  should work well for that layer. High divergence (&gt; 0.3, red) means the routing
  patterns differ significantly &mdash; preset may not help.
</div>
...
<div class="insight-box">
  <strong>How to read:</strong> Shows which experts are most frequently selected.
  Blue = training, orange = inference. Tall bars with both colors similar =
  stable expert preference. Tall bars where colors differ = preference shift.
</div>
"""
...
<h2 id="preset-coverage">Preset Circuit Coverage</h2>
<div class="chart-container">
  <img src="data:image/png;base64,{charts.get('preset_coverage', '')}" alt="Preset Coverage">
</div>
<div class="insight-box good">
  <strong>How to read:</strong> Green bars = rank pairs covered by preset circuits.
  Red bars = rank pairs NOT in the preset plan. High green coverage = preset
  circuits are well-placed. Lots of tall red bars = preset plan needs adjustment.
</div>
"""
